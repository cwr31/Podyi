...
struct SlidersView: View {
    ...
    private var individualComponentsSection: some View {
        VStack(alignment: .trailing, spacing: 20) {
            VerticalVolumeSlider(value: $volume, inRange: 0 ... maxVolume, activeFillColor: color, fillColor: normalFillColor, emptyColor: emptyColor, width: 8) { _ in
            }
            .frame(height: 130)

            CustomSlider(value: $sliderValue, inRange: 0 ... maxSliderValue, activeFillColor: color, fillColor: normalFillColor, emptyColor: emptyColor, height: 8) { _ in
            }
            .frame(height: 40)

            MusicProgressSlider(value: $playerDuration, inRange: TimeInterval.zero ... maxDuration, activeFillColor: color, fillColor: normalFillColor, emptyColor: emptyColor, height: 32) { _ in
            }
            .frame(height: 40)

            VolumeSlider(value: $volume, inRange: 0 ... maxVolume, activeFillColor: color, fillColor: normalFillColor, emptyColor: emptyColor, height: 8) { _ in
            }
            .frame(height: 30)
